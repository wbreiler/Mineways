...
wprintf(L"Really processed %s\n", gFG.fr[fullIndex].fullFilename);
			...
			if (chest_exists && (gChestGrid.chestCount > 0))
			{
				...
				if (gChestGrid.cr[CHEST_COPPER + catIndex * gChestGrid.totalTiles].exists ||
					...
					int count = 0;
					// Always iterate all TOTAL_CHEST_TILES (17) slots in transferChestData below.
					// We skip data for "unused" slots here (e.g., normal_double when 1.15+ split chests are in use,
					// or normal_left/right when only legacy double chests are in use). transferChestData's per-iteration
					// chestGrid.cr[...].exists check ignores chests that aren't present, so the skipped slots are harmless.
					// Previously numChests was TOTAL_CHEST_TILES-1 or -2, which silently dropped the last 1 or 2
					// copper_weathered_* chests from processing — the file was discovered but never used,
					// causing an unexplained off-by-one (or off-by-two) in the "files used" summary.
					numChests = TOTAL_CHEST_TILES;
					if (doubled) {
		...
		// _y.png files are synthesized emission tiles produced by Mineways or ChannelMixer.
		// TileMaker does not consume them; they are intended to be re-imported by Mineways.
		else if (catIndex == CATEGORY_SYNTHETIC) {
			for (index = 0; index < gFG.totalTiles; index++) {
				fullIndex = catIndex * gFG.totalTiles + index;
				if (gFG.fr[fullIndex].exists) {
					wprintf(L"WARNING: File '%s' is a synthesized '_y' tile (typically generated by Mineways or ChannelMixer)\n    and is not consumed by TileMaker, so it is being skipped.\n", gFG.fr[fullIndex].fullFilename);
					gWarningCount++;
					continue;
				}
			}
